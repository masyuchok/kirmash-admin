...
function formatSupplierExpenseOptionLabel(option: {
  ...
  const units =
    option.totalProductUnits > 0
      ? `${option.totalProductUnits} ад.`
      : 'без тавараў';
  return [
    formatDate(option.expenseDateUtc),
    title,
    formatAmount(option.grossAmount),
    units,
  ].join(' · ');
}
...
function formatItemTitleForPolishInvoice(
  titleRaw: string,
  ...
  if (isBookmark) {
    const cleanedBookmarkTitle = title
      .replace(
        /^(zak[łl]adka do ksi[aą]?[żz]ek|zak[łl]adka do ksi[aą]?[żz]ki)\s*/i,
        ''
      )
...
const CITY_COUNTRY_LABELS: Array<{ pattern: RegExp; country: string }> = [
  {
    pattern:
      /wrocław|wroclaw|warszawa|kraków|krakow|gdańsk|gdansk|poznań|poznan|łódź|lodz/i,
    country: 'Polska',
  },
  {
    pattern: /vilnius|wilno|kaunas|kowno|klaipėda|klaipeda|šiauliai|siauliai/i,
    country: 'Litwa',
  },
  ...
  {
    pattern:
      /amsterdam|rotterdam|haga|den haag|'s-gravenhage|gravenhage|utrecht/i,
    country: 'Holandia',
  },
  {
    pattern:
      /berlin|münchen|munich|hamburg|frankfurt|köln|cologne|dresden|leipzig/i,
    country: 'Niemcy',
  },
  ...
  {
    pattern: /wien|vienna|salzburg|innsbruck|graz|linz|unterweitersdorf/i,
    country: 'Austria',
  },
  {
    pattern: /bruxelles|brussels|antwerp|antwerpen|ghent|gent|liège|liege/i,
    country: 'Belgia',
  },
  {
    pattern: /zürich|zurich|bern|geneva|genève|geneve|basel/i,
    country: 'Szwajcaria',
  },
  {
    pattern: /rome|roma|milan|milano|naples|napoli|turin|torino/i,
    country: 'Włochy',
  },
  {
    pattern: /madrid|barcelona|valencia|seville|sevilla/i,
    country: 'Hiszpania',
  },
  ...
  {
    pattern: /london|manchester|birmingham|edinburgh|glasgow/i,
...
function invoiceCountryLabel(codeOrName?: string, address?: string): string {
  ...
  if (fromAddress) {
    if (fromAddress.length === 2) {
      return (
        INVOICE_COUNTRY_LABELS[fromAddress.toUpperCase()] ??
...
function addressIncludesCountry(address: string, country: string): boolean {
  ...
  const codeEntry = Object.entries(INVOICE_COUNTRY_LABELS).find(
    ([, label]) => label.toLowerCase() === normalizedCountry
  );
  if (codeEntry && normalizedAddress.includes(codeEntry[0].toLowerCase()))
    return true;
  const englishEntry = Object.entries(INVOICE_COUNTRY_NAME_ALIASES).find(
    ([, label]) => label.toLowerCase() === normalizedCountry
  );
...
function findPrimaryReportId(
  ...
  const match = periods.find(
    (period) => period.periodYear === year && period.periodMonth === month
  );
...
function buildCatalogPickerLines(
  ...
  return lines.sort((a, b) =>
    buildExpensePickerLineTitle(a).localeCompare(
      buildExpensePickerLineTitle(b),
      'be'
    )
  );
}
...
function buildExpensePickerLineTitle(line: {
  ...
  const base = formatProductNameWithAuthor(
    line.productName,
    line.productAuthor
  );
...
function resolveUnpaidVariantName(
  ...
  const variantName =
    product?.variants
      ?.find(
        (variant) =>
          normalizeShopifyNumericId(variant.variantId) === targetVariantId
      )
      ?.variantName?.trim() ?? '';
...
function buildExpensePickerLinesFromSupply(
  ...
  const catalogById = new Map(
    catalogProducts.map((product) => [product.shopifyProductId, product])
  );
  ...
  return supplyRows
    .map((row) => {
      ...
      return {
        ...
        shopifyVariantId: variantId,
        productName:
          product?.productName || row.productName || row.shopifyProductId,
        productAuthor: product?.productAuthor ?? '',
        variantName,
...
function syncExpenseAmounts(
  ...
  if (edited === 'vat') {
    ...
    if (net > 0)
      return { grossAmount: round2(vat + net), vatAmount: vat, netAmount: net };
    return values;
  }
  if (gross > 0)
    return {
...
function calcExpenseProductGrossTotal(
  lines: ExpenseProductLineDraft[]
): number {
  return round2(
    lines.reduce((sum, line) => sum + line.quantity * line.unitGrossPrice, 0)
  );
}
...
export default function ReportDetailsClient({
  ...
  const [foreignOrderRows, setForeignOrderRows] = useState<
    VatReportDetails['rows']
  >([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [expandedOrderId, setExpandedOrderId] = useState<string | null>(null);
  const [expandedForeignOrderId, setExpandedForeignOrderId] = useState<
    string | null
  >(null);
  const [expandedPolandRowId, setExpandedPolandRowId] = useState<number | null>(
    null
  );
  const [regeneratingRowKey, setRegeneratingRowKey] = useState<string | null>(
    null
  );
  const [pendingRegenerateRowKey, setPendingRegenerateRowKey] = useState<
    string | null
  >(null);
  const [editingRowKey, setEditingRowKey] = useState<string | null>(null);
  const [deletingRowKey, setDeletingRowKey] = useState<string | null>(null);
  const [updatingItemVatId, setUpdatingItemVatId] = useState<number | null>(
    null
  );
  const [updatingShippingVatRowId, setUpdatingShippingVatRowId] = useState<
    number | null
  >(null);
  ...
  const [pendingMoveToForeignRow, setPendingMoveToForeignRow] = useState<{
    rowId: number;
    rowKey: string;
  } | null>(null);
  const [moveToForeignName, setMoveToForeignName] = useState('');
  const [moveToForeignAddress, setMoveToForeignAddress] = useState('');
  const [movingToForeignRowKey, setMovingToForeignRowKey] = useState<
    string | null
  >(null);
  ...
  const [expenseGrossOverride, setExpenseGrossOverride] = useState<
    number | null
  >(null);
  const [expenseVatOverride, setExpenseVatOverride] = useState<number | null>(
    null
  );
  const [editingExpenseId, setEditingExpenseId] = useState<number | null>(null);
  const [editingExpenseInvoiceFileName, setEditingExpenseInvoiceFileName] =
    useState<string | null>(null);
  const [addMode, setAddMode] = useState<'select' | 'manual'>('select');
  const [sourceOrderOptions, setSourceOrderOptions] = useState<
    VatReportSourceOrderOption[]
  >([]);
  ...
  const [foreignProductLines, setForeignProductLines] = useState<
    ExpenseProductLineDraft[]
  >([]);
  const [foreignShopifyProducts, setForeignShopifyProducts] = useState<
    ProductWithSuppliers[]
  >([]);
  const [foreignShopifyProductsLoading, setForeignShopifyProductsLoading] =
    useState(false);
  ...
  const [deletingCashSaleId, setDeletingCashSaleId] = useState<number | null>(
    null
  );
  ...
  const [unpaidCatalogProducts, setUnpaidCatalogProducts] = useState<
    ProductWithSuppliers[]
  >([]);
  const [cashProductSearch, setCashProductSearch] = useState('');
  const [unpaidLinkModalOpen, setUnpaidLinkModalOpen] = useState(false);
  const [unpaidLinkTarget, setUnpaidLinkTarget] =
    useState<VatReportUnpaidProductRow | null>(null);
  const [unpaidLinkMode, setUnpaidLinkMode] = useState<'replace' | 'link'>(
    'replace'
  );
  const [unpaidLinkOptions, setUnpaidLinkOptions] =
    useState<VatReportUnpaidLinkOptions | null>(null);
  const [unpaidLinkOptionsLoading, setUnpaidLinkOptionsLoading] =
    useState(false);
  ...
  const [selectedOverpaidProductId, setSelectedOverpaidProductId] =
    useState('');
  const [unpaidLinkSource, setUnpaidLinkSource] = useState<
    'invoice' | 'payment'
  >('invoice');
  const [selectedInvoiceExpenseId, setSelectedInvoiceExpenseId] = useState('');
  const [selectedPaymentRecordExpenseId, setSelectedPaymentRecordExpenseId] =
    useState('');
  ...
  const loadCombinedDetails = (baseReportId: number) =>
    fetchVatReportCombinedDetails(baseReportId);
  ...
  const handleToggleLock = useCallback(async () => {
    ...
    try {
      ...
      setReportPeriods((prev) =>
        prev.map((period) =>
          period.periodYear === data.periodYear &&
          period.periodMonth === data.periodMonth
            ? {
                ...period,
                isLocked: nextLocked,
                reports: period.reports.map((report) => ({
                  ...report,
                  isLocked: nextLocked,
                })),
              }
  ...
  useEffect(() => {
    const monthYearTitle = data
      ? formatMonthYearBe(data.periodMonth, data.periodYear)
      : 'Справаздача';
    ...
    adjacentReportIds.next,
    adjacentReportIds.prev,
    data,
    router,
    setTopbarButtons,
    setTopbarPage,
  ...
  useEffect(() => {
    ...
    loadCombinedDetails(reportId)
      ...
      .catch((err: unknown) => {
        if (!cancelled)
          setError(
            err instanceof Error ? err.message : 'Памылка загрузкі справаздачы'
          );
      })
      .finally(() => {
  ...
  const isSupplierPaymentExpense = useMemo(() => {
    const selectedType = expenseTypes.find(
      (t) => t.id === newExpense.expenseInvoiceTypeId
    );
  ...
  const expenseSupplierIsVatPayer = useMemo(() => {
    ...
    return (
      expenseSuppliers.find((supplier) => supplier.id === expenseSupplierId)
        ?.isVatPayer ?? false
    );
  ...
  const expenseProductVatTotal = useMemo(
    () =>
      calcExpenseProductVatTotal(
        expenseProductLines,
        expenseSupplierId,
        expenseSupplierIsVatPayer
      ),
  ...
  useEffect(() => {
    ...
    const loadExpensePickerProducts = async (): Promise<
      ExpensePickerLine[]
    > => {
  ...
  const cashPickerLines = useMemo(
    () => buildCatalogPickerLines(cashProducts),
    [cashProducts]
  );
  ...
  const activeDetailsPanel = useMemo<
    'poland' | 'foreign' | 'expense' | 'cash' | 'unpaid' | null
  >(() => {
    ...
    if (expandedRow.type === 'foreign' && foreignOrderRows.length > 0)
      return 'foreign';
    ...
    try {
      ...
      if (data) {
        ...
        const match = allReports.find(
          (r) =>
            r.periodYear === data.periodYear &&
            r.periodMonth === data.periodMonth &&
            r.type === targetType
        );
        if (match) {
  ...
  const formatSourceOrderLabel = (option: VatReportSourceOrderOption) => {
    const baseNumber =
      option.orderNumber.split(' || ')[0]?.trim() || option.orderNumber;
  ...
  const submitAddRow = async () => {
    ...
    try {
      ...
      setAddRowError(
        err instanceof Error ? err.message : 'Памылка дадання радка справаздачы'
      );
    } finally {
      setAddingRow(false);
    }
  ...
  const toggleForeignPickerLine = (
    line: CatalogPickerLine,
    selected: boolean
  ) => {
    const product = foreignShopifyProducts.find(
  ...
  const updateForeignProductQuantity = (lineKey: string, quantity: number) => {
    ...
    setForeignProductLines((prev) =>
      prev.map((line) =>
        line.lineKey === lineKey ? { ...line, quantity: safeQuantity } : line
      )
    );
  };

  const updateForeignProductUnitPrice = (
    lineKey: string,
    unitGrossPrice: number
  ) => {
  ...
  const submitForeignRow = async () => {
    ...
    try {
      const shopifyOrderId = await createVatReportForeignRow(reportId, {
        ...
        shippingGrossAmount: Math.max(
          0,
          Number(newForeignRow.shippingGrossAmount) || 0
        ),
      ...
      setForeignAddError(
        err instanceof Error ? err.message : 'Памылка дадання замежнага радка'
      );
    } finally {
  ...
  const openExpenseForEdit = async (expense: VatReportExpenseRow) => {
    ...
    const supplierIsVatPayer =
      suppliers.find((supplier) => supplier.id === supplierId)?.isVatPayer ??
      false;
    const lines: ExpenseProductLineDraft[] = expense.products.map(
      (product) => ({
    ...
    const calcVat = calcExpenseProductVatTotal(
      lines,
      supplierId,
      supplierIsVatPayer
    );
    ...
    setExpenseProductSearch('');
    setExpenseGrossOverride(
      expense.grossAmount > productGross + 0.009 ? expense.grossAmount : null
    );
    setExpenseVatOverride(
      Math.abs(expense.vatAmount - calcVat) > 0.009 ? expense.vatAmount : null
    );
    setNewExpense({
  ...
  const toggleExpenseProduct = (
    pickerLine: ExpensePickerLine,
    selected: boolean
  ) => {
    if (selected) {
      ...
      setExpenseProductLines((prev) => {
        if (prev.some((line) => line.lineKey === pickerLine.lineKey))
          return prev;
        return [
          ...prev,
          {
    ...
    setExpenseProductLines((prev) =>
      prev.filter((line) => line.lineKey !== pickerLine.lineKey)
    );
  };
  ...
  const updateExpenseProductUnitGrossPrice = (
    lineKey: string,
    unitGrossPrice: number
  ) => {
    const safePrice = Math.max(0, unitGrossPrice);
  ...
  const submitExpense = async () => {
    ...
    if (
      newExpense.grossAmount < 0 ||
      newExpense.vatAmount < 0 ||
      newExpense.netAmount < 0
    ) {
    ...
    try {
      const payload = {
        ...
        expenseDateUtc: new Date(
          `${newExpense.expenseDateUtc}T00:00:00.000Z`
        ).toISOString(),
        ...
        supplierId:
          isSupplierPaymentExpense && expenseSupplierId > 0
            ? expenseSupplierId
            : undefined,
      ...
      if (editingExpenseId !== null) {
        ...
        if (expenseInvoiceFile) {
          await uploadVatReportExpenseInvoice(
            editingExpenseId,
            expenseInvoiceFile
          );
        }
  ...
  const downloadExpenseInvoice = async (expenseId: number) => {
    try {
      const { blob, fileName } =
        await downloadVatReportExpenseInvoice(expenseId);
      const url = URL.createObjectURL(blob);
  ...
  const [expensePaidSavingId, setExpensePaidSavingId] = useState<number | null>(
    null
  );
  ...
  const submitCashSale = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Памылка дадання наяўнай продажы'
      );
    } finally {
      setCashSaving(false);
    }
    ...
    try {
      ...
      if (options.overpaidProducts.length > 0) {
        ...
        if (options.overpaidProducts.length === 1) {
          setSelectedOverpaidProductId(
            String(options.overpaidProducts[0].expenseProductId)
          );
        }
      } else {
        ...
        if (options.supplierInvoices.length > 0) {
          ...
          if (options.supplierInvoices.length === 1) {
            setSelectedInvoiceExpenseId(
              String(options.supplierInvoices[0].expenseId)
            );
          }
          ...
          if (options.supplierPaymentRecords.length === 1) {
            setSelectedPaymentRecordExpenseId(
              String(options.supplierPaymentRecords[0].expenseId)
            );
          }
        }
      }
      ...
      setUnpaidLinkError(
        err instanceof Error ? err.message : 'Памылка загрузкі варыянтаў'
      );
    } finally {
  ...
  const submitUnpaidLink = async () => {
    ...
    const linkExpenseId =
      unpaidLinkMode === 'link'
        ? Number(
            unpaidLinkSource === 'invoice'
              ? selectedInvoiceExpenseId
              : selectedPaymentRecordExpenseId
          )
        : undefined;
    ...
    try {
      await linkUnpaidProduct({
        periodYear: (() => {
          if (unpaidLinkTarget.saleOrderDateUtc) {
            ...
            if (!Number.isNaN(saleDate.getTime()))
              return saleDate.getUTCFullYear();
          }
        ...
        expenseProductId:
          unpaidLinkMode === 'replace'
            ? Number(selectedOverpaidProductId)
            : undefined,
      ...
      setUnpaidLinkError(
        err instanceof Error ? err.message : 'Памылка прывязкі'
      );
    } finally {
  ...
  const removeCashSale = async (cashSaleId: number) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Памылка выдалення наяўнай продажы'
      );
    } finally {
  ...
  const removeExpense = async (expenseId: number) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Памылка выдалення расходу'
      );
    } finally {
  ...
  const handleUploadInvoice = async (rowId: number) => {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : 'Памылка загрузкі фактуры'
        );
      }
    };
    input.click();
  ...
  const handleExportTableToPdf = () => {
    ...
    const printableBody =
      printableTable.tBodies[0] ?? printableTable.createTBody();
...
<html>
  ...
  const handleExportForeignOrderToPdf = async (
    row: VatReportDetails['rows'][number]
  ) => {
    const uploadedInvoiceRow = row.polandRows.find((group) =>
      Boolean(group.invoiceFileName)
    );
    if (uploadedInvoiceRow) {
      setError(null);
      try {
        const { blob } = await downloadVatReportRowInvoice(
          uploadedInvoiceRow.id
        );
    ...
    const invoiceNumber = getInvoiceNumberForFile(
      row.name,
      `order-${row.shopifyOrderId || reportId}`
    );
    let invoiceSettings: {
      companyName: string;
      address: string;
    ...
    const itemRows = row.polandRows.flatMap((group) =>
      group.items.map((item) => {
        ...
        const vatAmount =
          rate > 0 ? round2((item.grossAmount * rate) / (1 + rate)) : 0;
        const netAmount = round2(item.grossAmount - vatAmount);
        return {
          title: formatItemTitleForPolishInvoice(
            item.productTitle,
            item.productType
          ),
    ...
    const shippingHtml = shippingRows
      ...
      .join('');
    const vatByRate = row.polandRows.reduce((acc, group) => {
  ...
  const confirmDeleteRow = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : 'Памылка выдалення радка справаздачы'
      );
    } finally {
  ...
  const confirmMoveRowToForeign = async () => {
    ...
    setError(null);
    try {
      await moveVatReportRowToForeign({
        rowId,
        deliveryName: name,
      ...
      setError(
        err instanceof Error ? err.message : 'Памылка пераносу радка ў замежныя'
      );
    } finally {
  ...
  const handleUpdateForeignItemVat = async (
    itemId: number,
    vatRatePercent: number
  ) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Памылка абнаўлення VAT па тавары'
      );
    } finally {
  ...
  const handleUpdateForeignShippingVat = async (
    ...
    try {
      ...
      const shippingCalc = recalcVatAndNet(
      ...
      setError(
        err instanceof Error
          ? err.message
          : 'Памылка абнаўлення VAT па дастаўцы'
      );
    } finally {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-6">
                    ...
                    onClick={() => {
                      if (
                        ...
                        setExpandedOrderId((prev) => {
                          const next =
                            prev === row.shopifyOrderId
                              ? null
                              : row.shopifyOrderId;
                          if (next !== null) {
                            if (row.type !== 'poland')
                              setExpandedPolandRowId(null);
                            if (row.type !== 'foreign')
                              setExpandedForeignOrderId(null);
                          }
                  ...
                  >
                    ...
                    row.shopifyOrderId !== 'foreign-summary' ? (
                          ...
                          <div className="inline-flex items-center gap-2">
                            <button
                              ...
                              onClick={(e) => {
                                ...
                                if (targetRowId)
                                  void handleUploadInvoice(targetRowId);
                              }}
                              disabled={isLocked}
                              ...
                              title={
                                isLocked ? lockedTitle : 'Загрузіць фактуру'
                              }
                            >
                      ...
                      <>
                        ...
                        <td className="px-4 py-3 text-right tabular-nums">
                          {row.type === 'unpaid' ? (
                            <span title="Ацэнка сабакошту па пастаўках">
                              {formatAmount(row.grossAmount ?? 0)}
                            </span>
                          ) : (
                            formatAmount(row.grossAmount ?? 0)
                            ...
                            row.type !== 'unpaid' && (
                              <button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  if (isLocked) return;
                                  setPendingRegenerateRowKey(
                                    `${row.type}-${row.shopifyOrderId}`
                                  );
                                }}
                                disabled={
                                  isLocked ||
                            ...
                            <tbody className="divide-y divide-gray-100">
                              {row.polandRows.flatMap((group) =>
                                group.items.map((item, idx) => {
                                  const rate =
                                    item.assignedVatRatePercent / 100;
                                  const vatAmount =
                                    rate > 0
                                      ? round2(
                                          (item.grossAmount * rate) / (1 + rate)
                                        )
                                      : 0;
                                  const netAmount = round2(
                                    item.grossAmount - vatAmount
                                  );
                                    ...
                                    <tr key={`${group.id}-${idx}`}>
                                      ...
                                      <td className="px-2 py-1.5 text-right">
                                        <div className="inline-flex items-center justify-end gap-2">
                                          <select
                                            value={String(
                                              item.assignedVatRatePercent
                                            )}
                                            onChange={(e) => {
                                              const nextVat = Number(
                                                e.target.value
                                              );
                                              if (!Number.isFinite(nextVat))
                                                return;
                                              void handleUpdateForeignItemVat(
                                                item.id,
                                                nextVat
                                              );
                                            }}
                              ...
                              {row.polandRows
                                .filter(
                                  (group) => group.shippingGrossAmount > 0
                                )
                                .map((group) => (
                                  <tr
                                    key={`shipping-${group.id}`}
                                    className="bg-white"
                                  >
                                    ...
                                    <td className="px-2 py-1.5 text-right tabular-nums">
                                      {formatAmount(group.shippingNetAmount)}
                                    </td>
                                    <td className="px-2 py-1.5 text-right">
                                      <div className="inline-flex items-center justify-end gap-2">
                                        <select
                                          ...
                                          onChange={(e) => {
                                            ...
                                            if (!Number.isFinite(nextVat))
                                              return;
                                            void handleUpdateForeignShippingVat(
                                              group,
                                              nextVat
                                            );
                                          }}
      ...
      {activeDetailsPanel && (
        <div
          ref={detailsPanelRef}
          className="w-full max-w-full scroll-mt-4 [overflow-anchor:none]"
        >
                            ...
                            <div className="absolute right-0 top-full z-20 mt-1.5 w-36 rounded-lg border border-gray-200 bg-white p-2 text-left shadow-lg">
                              <label className="flex items-center gap-2 rounded-md px-2 py-1.5 text-xs font-medium normal-case tracking-normal text-gray-700 hover:bg-gray-50">
                                <input
                                  ...
                                  checked={vatFilter5}
                                  onChange={(e) =>
                                    setVatFilter5(e.target.checked)
                                  }
                              ...
                              <label className="mt-1 flex items-center gap-2 rounded-md px-2 py-1.5 text-xs font-medium normal-case tracking-normal text-gray-700 hover:bg-gray-50">
                                <input
                                  ...
                                  checked={vatFilter23}
                                  onChange={(e) =>
                                    setVatFilter23(e.target.checked)
                                  }
                                  className="size-3.5 rounded border-gray-300 accent-primary"
                                />
                                23%
                              ...
                              <td className="px-4 py-3 text-right tabular-nums">
                                {isEditing ? (
                                  <input
                                    ...
                                    step="0.01"
                                    value={
                                      edited?.grossAmount ?? row.grossAmount
                                    }
                                    onChange={(e) => {
                                      const value = Number(e.target.value) || 0;
                                      setEditedRows((prev) => ({
                              ...
                              <td className="px-4 py-3 text-right">
                                <div className="inline-flex items-center gap-2">
                                        ...
                                        if (edited) {
                                          try {
                                            await updateVatReportRow({
                                              ...
                                              shippingGrossAmount:
                                                edited.shippingGrossAmount,
                                            });
                                            const { details, foreignRows } =
                                              await loadCombinedDetails(
                                                reportId
                                              );
                                            setForeignOrderRows(foreignRows);
                                            setData(details);
                                  ...
                                  <button
                                    ...
                                    disabled={
                                      isLocked || deletingRowKey === rowKey
                                    }
                                    className="inline-flex size-8 items-center justify-center rounded-full border border-red-200 bg-white text-red-600 shadow-sm transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-40"
                                    aria-label="Выдаліць радок"
                                    title={
                                      isLocked ? lockedTitle : 'Выдаліць радок'
                                    }
                                  >
                            ...
                            <tr className="bg-gray-50/50">
                              ...
                              >
                                {row.items.map((item, itemIdx) => (
                                  <div
                                    key={`${item.productTitle}-${itemIdx}`}
                                    className="py-0.5"
                                  >
                                    {item.productTitle} · qty {item.quantity} ·
                                    type: {item.productType || '—'} · VAT{' '}
                                    {formatAmount(item.assignedVatRatePercent)}%
                                ...
                                <table className="min-w-full border-collapse text-left text-xs">
                                  ...
                                  <tbody className="divide-y divide-gray-100">
                                    {row.polandRows.map((group) => {
                                      ...
                                      const grossAmount = isEditing
                                        ? (edited?.grossAmount ??
                                          group.grossAmount)
                                        : group.grossAmount;
                                      const vatAmount = isEditing
                                        ? (edited?.vatAmount ?? group.vatAmount)
                                        : group.vatAmount;
                                      const netAmount = isEditing
                                        ? (edited?.netAmount ?? group.netAmount)
                                        : group.netAmount;
                ...
                <tbody className="divide-y divide-gray-100">
                  {visibleCashRows.map((sale) => (
                    <tr key={`cash-${sale.id}`}>
                      ...
                      <td className="px-4 py-3 text-right tabular-nums">
                        {formatAmount(sale.unitPrice)}
                      </td>
                      <td className="px-4 py-3 text-right tabular-nums">
                        {formatAmount(sale.grossAmount)}
                      </td>
                      <td className="px-4 py-3 text-right">
                        <button
                          type="button"
            ...
            <div className="w-full overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
              <div className="border-b border-gray-100 px-6 py-4">
                ...
                <p className="mt-1 text-xs text-gray-500">
                  Проданы ў гэтым месяцы (па даце продажы), але фактура
                  пастаўшчыку не пакрыла гэты тавар. Прывязаныя да аплаты
                  пазначаны нумарам фактуры.
                </p>
              </div>
              <table className="w-full border-collapse text-left text-sm">
                <thead>
                ...
                <tbody className="divide-y divide-gray-100">
                  {visibleUnpaidRows.map((item) => {
                    const productLabel = formatUnpaidProductLabel(
                      item,
                      unpaidCatalogProducts
                    );
                    return (
                      <tr
                        key={`unpaid-${item.shopifyProductId}-${item.shopifyVariantId ?? ''}-${item.supplierId ?? 0}-${item.linkedExpenseId ?? 0}`}
                        className={
                          item.isManuallyLinked ? 'bg-sky-50/60' : undefined
                        }
                      >
                        <td className="px-4 py-3">
                          {item.saleOrderDateUtc
                            ? formatDate(item.saleOrderDateUtc)
                            : '—'}
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3 text-right tabular-nums">
                          {item.unitSupplyPrice > 0
                            ? formatAmount(item.unitSupplyPrice)
                            : '—'}
                        </td>
                        <td className="px-4 py-3 text-right tabular-nums">
                        ...
                        <td className="px-4 py-3 text-right">
                          {item.isManuallyLinked ? (
                            ...
                            <button
                              ...
                              title={
                                isLocked ? lockedTitle : 'Прывязаць да фактуры'
                              }
                            >
                              Прывязаць
                            </button>
              ...
              <table className="w-full border-collapse text-left text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {visibleExpenseRows.map((expense) => {
                    const rowHighlightClass = expense.invoiceFileName
                      ? 'bg-emerald-200/60 font-medium'
                      : '';
                    return (
                      <tr
                      ...
                      >
                        <td className={`px-4 py-3 ${rowHighlightClass}`}>
                          <div className="space-y-1">
                            <div className="inline-flex items-center gap-2">
                              <span>
                                {expense.expenseInvoiceTypeName || '—'}
                              </span>
                            ...
                            {expense.supplierName && (
                              <div className="text-xs text-gray-500">
                                Пастаўшчык: {expense.supplierName}
                              </div>
                            )}
                            {expense.invoiceNumber && (
                              <div className="text-xs text-gray-500">
                                № фактуры: {expense.invoiceNumber}
                              </div>
                            )}
                        ...
                        <td className={`px-4 py-3 ${rowHighlightClass}`}>
                          {formatDate(expense.expenseDateUtc)}
                        </td>
                        <td
                          className={`px-4 py-3 text-right tabular-nums ${rowHighlightClass}`}
                        >
                          {formatAmount(expense.netAmount)}
                        </td>
                        <td
                          className={`px-4 py-3 text-right tabular-nums ${rowHighlightClass}`}
                        >
                          {formatAmount(expense.vatAmount)}
                        </td>
                        <td
                          className={`px-4 py-3 text-right tabular-nums ${rowHighlightClass}`}
                        >
                        ...
                        <td
                          className={`max-w-[16rem] truncate px-4 py-3 ${rowHighlightClass}`}
                          title={expense.comment || undefined}
                        >
                          {expense.comment || '—'}
                        </td>
                        <td
                          className={`px-4 py-3 text-center ${rowHighlightClass}`}
                        >
                          <input
                            type="checkbox"
                            ...
                            disabled={
                              isLocked || expensePaidSavingId === expense.id
                            }
                            onChange={(e) =>
                        ...
                        <td
                          className={`px-4 py-3 text-right ${rowHighlightClass}`}
                        >
                          <div className="inline-flex items-center gap-2">
                            {expense.invoiceFileName && (
                              <button
                                type="button"
                                onClick={() =>
                                  void downloadExpenseInvoice(expense.id)
                                }
                            ...
                            <button
                              ...
                              disabled={
                                isLocked || deletingExpenseId === expense.id
                              }
                              className="inline-flex size-8 items-center justify-center rounded-full border border-red-200 bg-white text-red-600 shadow-sm transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-40"
              ...
              <div className="mt-4 space-y-2">
                <div className="flex items-center justify-between gap-3">
                  ...
                  {foreignProductLines.length > 0 && (
                    <span className="text-xs text-gray-500">
                      Выбрана: {foreignProductLines.length}
                    </span>
                  )}
                </div>
                <input
                ...
                <div className="max-h-[min(12rem,30vh)] overflow-y-auto rounded-lg border border-gray-200">
                  ...
                  {!foreignShopifyProductsLoading &&
                    visibleForeignPickerLines.length === 0 && (
                      <div className="px-3 py-4 text-sm text-gray-500">
                ...
                {foreignProductLines.length > 0 && (
                  <div className="rounded-lg bg-gray-50 px-3 py-2 text-sm text-gray-700">
                    ...
                    <span className="font-semibold tabular-nums">
                      {formatAmount(foreignProductGrossTotal)}
                    </span>
              ...
              <div className="space-y-3">
                <div className="space-y-2">
                  ...
                  <div className="max-h-[min(12rem,32vh)] overflow-y-auto rounded-lg border border-gray-200">
                    ...
                    {!cashProductsLoading &&
                      visibleCashPickerLines.length === 0 && (
                        <div className="px-3 py-4 text-sm text-gray-500">
                          Тавары не знойдзены
                        </div>
                      )}
                ...
                {newCashSale.lineKey &&
                  newCashSale.quantity > 0 &&
                  newCashSale.unitPrice > 0 && (
                    <div className="rounded-lg bg-gray-50 px-3 py-2 text-sm text-gray-700">
                      Сума:{' '}
                      <span className="font-semibold tabular-nums">
                        {formatAmount(
                          round2(newCashSale.quantity * newCashSale.unitPrice)
                        )}
                      </span>
                    </div>
                  )}
      ...
      {unpaidLinkModalOpen && unpaidLinkTarget && (
        <div className="fixed inset-0 z-[80] flex items-end justify-center overflow-y-auto bg-black/40 p-3 sm:items-center sm:p-4">
          <div className="my-auto flex max-h-[min(640px,calc(100dvh-1.5rem))] w-full max-w-2xl min-h-0 flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl sm:my-0">
            <div className="shrink-0 border-b border-gray-100 px-4 py-3 sm:px-5">
              <div className="text-base font-semibold text-gray-900">
                Прывязаць да фактуры
              </div>
              <p className="mt-1 text-sm text-gray-600">
                {(() => {
                  const label = formatUnpaidProductLabel(
                    unpaidLinkTarget,
                    unpaidCatalogProducts
                  );
                  ...
                  <div className="space-y-2">
                    ...
                    {unpaidLinkOptions.overpaidProducts.length === 0 && (
                      <p className="text-xs text-gray-500">
                        Пераплочаных тавараў у гэтага пастаўшчыка няма.
                      </p>
                    )}
                  </div>
                  ...
                  <div className="space-y-3">
                    <label className="flex cursor-pointer items-start gap-2 rounded-lg border border-gray-200 px-3 py-2.5">
                      ...
                      <span>
                        ...
                        <span className="mt-0.5 block text-xs text-gray-500">
                          Неаплочаны тавар пакрываецца выбранай фактурай або
                          іншым запісам аплаты.
                        </span>
                      </span>
                    </label>
                    {unpaidLinkMode === 'link' && (
                      <div className="space-y-3 pl-1">
                        <label className="block space-y-1.5">
                          ...
                          >
                            ...
                            {unpaidLinkOptions.supplierInvoices.map(
                              (option) => (
                                <option
                                  key={`invoice-${option.expenseId}`}
                                  value={String(option.expenseId)}
                                >
                          ...
                          {unpaidLinkOptions.supplierInvoices.length === 0 && (
                            <p className="text-xs text-gray-500">
                              Фактур з нумарам або файлам няма.
                            </p>
                          )}
                        </label>
                        ...
                        <label className="block space-y-1.5">
                          ...
                          >
                            ...
                            {unpaidLinkOptions.supplierPaymentRecords.map(
                              (option) => (
                                <option
                                  key={`payment-${option.expenseId}`}
                                  value={String(option.expenseId)}
                                >
                                  {formatSupplierExpenseOptionLabel(option)}
                                </option>
                          ...
                          {unpaidLinkOptions.supplierPaymentRecords.length ===
                            0 && (
                            <p className="text-xs text-gray-500">
      ...
      {expenseModalOpen && (
        <div className="fixed inset-0 z-[80] flex items-end justify-center overflow-y-auto bg-black/40 p-3 sm:items-center sm:p-4">
          <div className="my-auto flex max-h-[min(720px,calc(100dvh-1.5rem))] w-full max-w-2xl min-h-0 flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl sm:my-0">
            <div className="shrink-0 border-b border-gray-100 px-4 py-3 sm:px-5">
              <div className="text-base font-semibold text-gray-900">
                ...
                <label className="block space-y-1.5 sm:col-span-2">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setNewExpense((prev) => ({
                        ...prev,
                        expenseInvoiceTypeId,
                      }));
                      const nextType = expenseTypes.find(
                        (t) => t.id === expenseInvoiceTypeId
                      );
                  ...
                  <>
                    ...
                    <div className="space-y-2 sm:col-span-2">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setExpenseProductSearch(e.target.value)
                        }
                        placeholder="Пошук тавару..."
                        className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800 focus-visible:border-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/20"
                      />
                      <div className="max-h-[min(10rem,28vh)] overflow-y-auto rounded-lg border border-gray-200">
                        ...
                        {!supplierProductsLoading &&
                          visibleSupplierProducts.length === 0 && (
                            <div className="px-3 py-4 text-sm text-gray-500">
                              {expenseSupplierId > 0
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className="min-w-0 flex-1 basis-[12rem] text-sm text-gray-800">
                                  ...
                                  <span className="text-xs text-gray-500">
                                    VAT{' '}
                                    {formatAmount(pickerLine.vatRatePercent)}%
                                  </span>
                                </span>
                                {selected && (
                                  <div className="flex w-full shrink-0 flex-wrap items-center gap-2 sm:ml-auto sm:w-auto">
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white p-5 shadow-xl">
            ...
            <p className="mt-2 text-sm text-gray-600">
              Вы сапраўды хочаце выдаліць гэты радок справаздачы?
            </p>
            <div className="mt-5 flex items-center justify-end gap-2">
              <button
                type="button"
      ...
      {pendingMoveToForeignRow && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white p-5 shadow-xl">
            ...
            <p className="mt-2 text-sm text-gray-600">
              Увядзіце даныя для фактуры. Радок будзе перанесены з польскага ў
              замежны справаздачу.
            </p>
            ...
            <div className="mt-5 flex items-center justify-end gap-2">
              ...
              <button
                ...
                disabled={
                  !!movingToForeignRowKey ||
                  !moveToForeignName.trim() ||
                  !moveToForeignAddress.trim()
                }
                className="inline-flex min-w-24 items-center justify-center rounded-lg border border-primary bg-primary px-3 py-1.5 text-sm font-medium text-white transition hover:bg-primary/90 disabled:opacity-60"
              >
                {movingToForeignRowKey ? (
