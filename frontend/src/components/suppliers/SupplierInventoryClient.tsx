...
import {
  ...
  updateSupplierInventoryPricing,
...
import InventoryPricingCells, {
  InventoryPricingEditorProvider,
  InventoryPricingSaveButton,
  type InventoryPricingSaveValues,
} from '@/components/suppliers/InventoryPricingCells';
        ...
        .catch((err: unknown) => {
          setError(
            err instanceof Error
              ? err.message
              : 'Памылка загрузкі інвентарызацыі'
          );
        })
        .finally(() => {
          setLoading(false);
  ...
  const handleSavePricing = async (
    ...
    try {
      const updated = await updateSupplierInventoryPricing({
        ...
        syncWithShopify: values.syncWithShopify,
      });
      setRows((prev) =>
        prev.map((item) =>
          inventoryRowKey(item) === inventoryRowKey(row)
            ? {
                ...
                shopifyPrice: updated.shopifyPrice,
              }
            : item
        )
      );
      ...
      setExportNotice(
        err instanceof Error ? err.message : 'Памылка захавання цаны'
      );
    }
  };
  ...
  const filteredRows = useMemo(() => {
    ...
    if (showSupplierColumn && supplierOptions.length > 0) {
      filtered = filtered.filter((row) =>
        selectedSupplierIds.has(row.supplierId)
      );
    }
  ...
  const displayRows = useMemo(() => {
    ...
    const sortedGroups = [...groups].sort((a, b) => {
      if (sort) {
        ...
        return (
          (getGroupSortValue(a, sort.column) -
            getGroupSortValue(b, sort.column)) *
          direction
        );
      }
              ...
              {supplierOptions.map(([id, name]) => (
                ...
                >
                  <input
                    ...
                    onChange={(e) =>
                      toggleSupplierFilter(id, e.currentTarget.checked)
                    }
