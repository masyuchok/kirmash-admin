...
namespace backend.Services
{
    public class InventorySalesCacheService
    {
        ...
        public InventorySalesCacheService( AppDbContext db, ShopifyOrderFetchService shopifyOrders )
        {
            _db = db;
        ...
        {
            Dictionary<string, int> soldByProduct = await _shopifyOrders.GetSoldQuantitiesByProductFromShopifyAsync();
        ...
        private async Task RunIncrementalSyncAsync( InventorySalesSyncState state )
        {
            ...
            Dictionary<string, int> delta = await _shopifyOrders.GetSoldQuantitiesFromShopifySinceAsync( sinceUtc );
            if (delta.Count > 0)
            {
